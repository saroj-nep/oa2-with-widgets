...
function getfrompython()
{
    ...
    fs.writeFile('pythonoutput.txt', data, (err) => {
...
function msg_send() {
    // set typing to false and close the timer
    typingFalse();
...
function key_changer(e) {
    ...
    if (e == 37) {
        ...
        for (i = rcodes.length; i > 0; i--) {
            if (stpp == 1) { change_room(rcodes[i]); return }
            if (i == index) { stpp = 1 }
        } change_room(rcodes[rcodes.length - 1])
    }

}

// ----------
status_previous = undefined;
...
function pmarrnotify_add(x) {
    x = parseInt(x);
    if (pmarrnotify.indexOf(x) < 0) { pmarrnotify.push(x) }
}

function pmarrnotify_rem(x) {
    x = parseInt(x);
...
function manage_esc() {
    ...
    // disable esc key if in the settings
    let esc_disabled = Number(localStorage.getItem("eduTechEsc_disabled"));
    if (esc_disabled == 1) {
...
function swap_color(x, y, u) {
    ...
    if (u > 0) {
        settings_save('color', x); settings_save('colva', y)
...
function mdown(x) {
    if (allowselect < 1 && typeof x.preventDefault == 'function') {
        x.preventDefault()
    } dragdown = true; dragypos = x.pageY
}
...
function ttouch2(e) {
    ...
    if (globx < 50 && (currx - globx) > 50) {
        if (de('panel_pmlog').style.display == 'block') {
            hidepm_notify()
        } hst.style.display = 'block'; pab.style.display = 'block'; hoop(pan, 1); return
    }
...
// Ahmed's typing code here
let typing = false;
let typingPreviously = false;
let typingTimerId = undefined;
let typingTimerValue = Number(localStorage.getItem("eduTechIsTypingTimer"));
let typingTimerValueDefault = 3000;

if (typingTimerValue == undefined) { typingTimerValue = typingTimerValueDefault }
if (typingTimerValue > 60000 || typingTimerValue < 100) { typingTimerValue = typingTimerValueDefault }

function chatKeyPress(event) {
    // Get IsTyping settings
    if (Number(localStorage.getItem('eduTechIsTyping')) == false) { return }

    if (event.keyCode != 13) {
        if (typing != true) { change_status(6, "Typing") }
        typing = true;
        clearTimeout(typingTimerId);
        typingTimerId = setTimeout(typingFalse, typingTimerValue);

    }
    
}

function typingFalse() {
    typing = false;
    clearTimeout(typingTimerId);
    change_status(1, "Available")
}

// called in the online_construct function
function showTyping() {
    // Get IsTyping settings
    if (Number(localStorage.getItem('eduTechIsTyping')) == false) { return }

    let typing_users = [];
    let typing_users_html = "";

    for (i in usr_online) {
        if (usr_online[i][3] == 6) {
            if (usr_online[i][0] != myuuid) {
                typing_users.push(i);
                typing_users_html += usr_online[i][2] + " is typing" + "<br />";
            }
        }
    }
    document.getElementById("edutech_typing").innerHTML = typing_users_html;
}

let PREDICT_URL = "https://0ace-134-96-123-178.ngrok.io";

// To call below function use
// getPrediction ("hi", function (result) { document.getElementById("text_input").value=result } )
// Put any code that changes/updates the DOM elements based on the prediction in callback function

// Fetch sentence prediction
function getPrediction(inputText, callback) {
    fetch(PREDICT_URL + '?input=' + "'" + inputText + "'", {
        method: 'GET', // or 'PUT'
        headers: {
        ...
        .then(data => {
            data.data = data.data.replaceAll("\n", "").replaceAll("\r", "");
            callback(data.data);
