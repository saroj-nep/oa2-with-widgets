...
case 'messages'  : $tbl=$dbss['prfx'].'_messages'; break;
default        : break;}
...
if(isset($_POST['showroombg']) && isset($_POST['reference']) && isset($_POST['novalty']) && isset($_POST['usercontrol']) && isset($_POST['ttonoff']) && isset($_POST['amapponoff']) && isset($_POST['roombgc']) && isset($_POST['roombgf']) && isset($_POST['roombgt']) && isset($_POST['roombgs']) && isset($_POST['roombgl'])){  


/* EduTech ESC key disabled Settings code */	
$eduTechEsc_disabled=(int)$_POST['eduTechEsc_disabled'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$eduTechEsc_disabled' WHERE id='eduTechEsc_disabled'");	

/* EduTech Is Typing Settings code */	
$eduTechIsTyping=(int)$_POST['eduTechIsTyping'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$eduTechIsTyping' WHERE id='eduTechIsTyping'");	

/* EduTech Is Typing Timer Settings code */	
$eduTechIsTypingTimer=(int)$_POST['eduTechIsTypingTimer'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$eduTechIsTypingTimer' WHERE id='eduTechIsTypingTimer'");	
	

$showroombg=(int)$_POST['showroombg'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$showroombg' WHERE id='showroombg'");

/* sentence starter one part "reference"  onoff admin controller setings code*/	
$reference=(int)$_POST['reference'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$reference' WHERE id='reference'");	

/* sentence starter one part "novalty"  onoff admin controller setings code*/	
$novalty=(int)$_POST['novalty'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$novalty' WHERE id='novalty'");	

/* User grant access to sentence on off  code*/	
$usercontrol=(int)$_POST['usercontrol'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$usercontrol' WHERE id='usercontrol'");	
	
/* time triger option on off */	
$ttonoff=(int)$_POST['ttonoff'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$ttonoff' WHERE id='ttonoff'");
	
/* Arguement mining app onoff */	
$amapponoff=(int)$_POST['amapponoff'];   
neutral_query('UPDATE '.$dbss['prfx']."_settings SET value='$amapponoff' WHERE id='amapponoff'");
...
if(isset($_POST['color'])){$color=stripc($_POST['color']);}else{$color='666';}
	//update on off in the chate room
if(isset($_POST['onoff'])){$onoff=(int)$_POST['onoff'];}else{$onoff=1;}
if(isset($_POST['zorder'])){$zorder=(int)$_POST['zorder'];}else{$zorder=1001;}
if($zorder<1000){$zorder+=1000;} if($rid==1){$zorder=0;}

neutral_query('UPDATE '.$dbss['prfx']."_rooms SET name='$name',description='$desc',color='$color',onoff=$onoff,zorder=$zorder WHERE id=$rid");
